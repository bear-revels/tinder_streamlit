...
class Database:
    ...
    def refresh_images(self):
        ...
        for img_path in real_path.glob('*'):
            ...
            cursor.execute('INSERT OR IGNORE INTO creators (creator_name) VALUES (?)', (creator_name,))
            cursor.execute('SELECT creator_id FROM creators WHERE creator_name = ?', (creator_name,))
            creator_id = cursor.fetchone()[0]
            cursor.execute('INSERT OR IGNORE INTO images (creator_id, is_real, is_active, filepath) VALUES (?, 1, 1, ?)', (creator_id, str(img_path).replace('\\', '/')))
        ...
        for img_path in genai_path.glob('*'):
            ...
            cursor.execute('SELECT creator_id FROM creators WHERE creator_name = ?', (creator_name,))
            creator_id = cursor.fetchone()[0]
            cursor.execute('INSERT OR IGNORE INTO images (creator_id, is_real, is_active, filepath) VALUES (?, 0, 1, ?)', (creator_id, str(img_path).replace('\\', '/')))
    ...
    def get_random_images(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute('SELECT * FROM images WHERE is_active = 1 AND is_real = 1 ORDER BY RANDOM() LIMIT 1')
        real_image = cursor.fetchone()
        cursor.execute('SELECT * FROM images WHERE is_active = 1 AND is_real = 0 ORDER BY RANDOM() LIMIT 1')
    ...
    def record_score(self, player_id, level, image1_id, image2_id, selection):
        ...
        cursor = self.conn.cursor()
        cursor.execute('INSERT INTO scores (player_id, level, image1_id, image2_id, selection) VALUES (?, ?, ?, ?, ?)', (player_id, level, image1_id, image2_id, selection))
        self.conn.commit()
    ...
    def add_image(self, creator_name, is_real, filepath):
        ...
        cursor.execute('SELECT creator_id FROM creators WHERE creator_name = ?', (creator_name,))
        creator_id = cursor.fetchone()[0]
        cursor.execute('INSERT OR IGNORE INTO images (creator_id, is_real, is_active, filepath) VALUES (?, ?, 1, ?)', (creator_id, is_real, filepath.replace('\\', '/')))
        self.conn.commit()
